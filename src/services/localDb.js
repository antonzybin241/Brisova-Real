...
const STORAGE_KEY = "brisova-store-v1";
...
const persist = () => {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(dehydrate(state)));
  } catch {
...
const load = () => {
  ...
  try {
    ...
    if (raw) {
      state = { ...emptyState(), ...hydrate(JSON.parse(raw)) };
      return;
    }
  } catch {
  ...
  const seeded = hydrate(JSON.parse(JSON.stringify(seed)));
  state = { ...emptyState(), ...seeded };
  if (!Array.isArray(state.newsletter)) state.newsletter = [];
  persist();
};
...
const applyInclude = (doc, include, db) => {
  ...
  for (const [name, spec] of Object.entries(include)) {
    ...
    if (name === "owner" || (name === "user" && doc.userId)) {
      const related = db[name === "owner" ? "user" : "user"]
        ._rows()
        .find((row) => row.id === (name === "owner" ? doc.ownerId : doc.userId));
...
const DEFAULTS = {
  ...
  newsletter: { subscribed: true, source: "website" },
};
